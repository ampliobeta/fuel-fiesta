...
- MISSING PRODUCTS — SUGGEST DON'T INVENT: If the ride clearly needs a product type the rider hasn't selected, do not include it in the packing list or optional list. Suggestions go ONLY in resilience.warning as plain text pointing back to the kit. Examples: "No gels selected — for efforts this long you'll want something between stops. Consider adding a gel from the kit above." or "No electrolytes selected — a salt cap would help for this duration. Add one from the kit above." NEVER put unselected products in packing_list.required or packing_list.optional — not even as optional items.
- MATH HONESTY: If the selected products cannot physically deliver the target carbs for the ride duration, say so. Do not assign impossible carb totals to a single product. Example: one 500ml bottle of Formula 369 at 3 scoops = 90g carbs maximum — it cannot deliver 255g. If there is a gap between what's selected and what's needed, flag it clearly in resilience.warning. Never inflate a product's carb contribution beyond what it can physically hold.
- MATH HONESTY BOTH WAYS: If the packing list total carbs significantly exceed the target (more than 15% over), reduce quantities to match — use fewer gels, not a lower carb target. The carb/hr target is set by intensity and gut training and must never be reduced to match what was selected. Adjust quantities down to hit the target, not the target down to match the quantities.
